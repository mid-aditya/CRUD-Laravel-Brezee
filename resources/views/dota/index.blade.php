...
<body>
    <div class="container">
        <h1>Video List</h1>
        <a href="{{ route('dota.create') }}">Add New Video</a>        <a href="{{ route('dashboard') }}">Back</a>
        <table>
            <thead>
                <tr>
                    <th>Image</th>
            ...
            <tbody>
                @foreach($dota as $dotas)
                    <tr>
                        <td><img src="{{ $dotas->gambar }}" alt="{{ $dotas->judul }}"></td>
                        <td>{{ $dotas->judul }}</td>
                        <td class="actions">
                            <a href="{{ route('dota.edit', $dotas->id_videos) }}">Edit</a>
                            <form class="delete-form" action="{{ route('dota.destroy', $dotas->id_videos) }}" method="POST">
                                @csrf
                                @method('DELETE')
