...
<body>
    <div class="container">
        <h1>Video List</h1>
        <a href="{{ route('videos.create') }}">Add New Video</a>        <a href="{{ route('dashboard') }}">Back</a>
        <table>
            <thead>
                <tr>
                    <th>Image</th>
            ...
            <tbody>
                @foreach($videos as $video)
                    <tr>
                        <td><img src="{{ $video->gambar }}" alt="{{ $video->judul }}"></td>
                        <td>{{ $video->judul }}</td>
                        <td class="actions">
                            <a href="{{ route('videos.edit', $video->id_videos) }}">Edit</a>
                            <form class="delete-form" action="{{ route('videos.destroy', $video->id_videos) }}" method="POST">
                                @csrf
                                @method('DELETE')
